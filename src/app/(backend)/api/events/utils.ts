export default async function getAccessToken(): Promise<string> {
  ...
  const params = new URLSearchParams({
    ...
    refresh_token: process.env.GOOGLE_OAUTH_REFRESH, // getting refresh token is tricky()
  ...
  const json = await response.json(); // json also contains scope along with token,
  console.log("succes tokrn? ", json);
